...
df1b2variable dunif(const df1b2variable& x, const double min, const double max)
{
	return log(max-min);
}
...
df1b2variable dbeta(const df1b2variable& x, const double a, const double b)
{
	return - gammln(a+b)+(gammln(a)+gammln(b))-(a-1.)*log(x)-(b-1.)*log(1.-x);
}
...
df1b2variable dgamma(const df1b2variable &x, const double a, const double b)
{
	//E(x)=a/b;
	//V(x)=a/b^2
	return -a*log(b)+gammln(a)-(a-1)*log(x)+b*x;
...
df1b2variable dnorm(const df1b2vector& residual, const df1b2variable std)
{
	RETURN_ARRAYS_INCREMENT();
	double pi=3.141593;
	long n=size_count(residual);
...
dvariable dnorm(const dvar_matrix& residual, const double std)
{   
	RETURN_ARRAYS_INCREMENT();
	double pi=3.141593;
	long n=size_count(residual);
